...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        for(int j=0;j<player.length;j++) {
            ...
            // Check for diagonal
            if (grid[0][0] == player[j] && grid[1][1] == player[j] && grid[2][2] == player[j])
                return winner[j];
            if (grid[0][2] == player[j] && grid[1][1] == player[j] && grid[2][0] == player[j])
                return winner[j];

            for(int i=0;i<grid.length;i++) {
                // Check for rows
                if (grid[0][i] == player[j] && grid[1][i] == player[j] && grid[2][i] == player[j])
                    return winner[j];

                // Check for columns
                if (grid[i][0] == player[j] && grid[i][1] == player[j] && grid[i][2] == player[j])
                    return winner[j];
            }
